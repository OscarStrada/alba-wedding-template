/**
 * Backend del buscador de confirmación de asistencia (RSVP) — Valentina & Emilio.
 *
 * ─── CÓMO DESPLEGAR ─────────────────────────────────────────────────────────
 * 1. Crea una hoja de cálculo nueva en Google Sheets y pega ahí los datos de
 *    invitados (ver google-apps-script/demo-guests.csv para el formato).
 * 2. En la hoja: Extensiones → Apps Script.
 * 3. Borra el contenido de Code.gs que abre por defecto y pega este archivo
 *    completo en su lugar.
 * 4. Ajusta SHEET_NAME abajo si tu pestaña no se llama "Hoja1".
 * 5. Implementar → Nueva implementación → Tipo: App web
 *      - Ejecutar como: Yo (tu cuenta)
 *      - Quién tiene acceso: Cualquier usuario
 * 6. Autoriza los permisos cuando se solicite.
 * 7. Copia la URL de implementación y pégala en VITE_GOOGLE_SCRIPT_URL dentro
 *    de un archivo .env en la raíz del proyecto (ver RSVP-SETUP.md).
 * 8. Vuelve a construir el sitio (npm run build) para que Vite recoja la
 *    variable de entorno.
 *
 * Este script solo BUSCA y ACTUALIZA filas existentes — nunca agrega filas
 * nuevas a la hoja. Los invitados y su cupo (acompañante/niños) deben existir
 * de antemano en la hoja.
 *
 * ─── COLUMNAS ESPERADAS (fila 1 = encabezados) ─────────────────────────────
 *   A: # (no se usa)          F: Acompañante (cupo asignado, solo lectura)
 *   B: Nombre                 G: Asistirá     ("Sí" / "No")
 *   C: Apellido paterno       H: Niños        (cupo asignado, solo lectura)
 *   D: Apellido materno
 *   E: Familia
 * ────────────────────────────────────────────────────────────────────────────
 */
  ...
  try {
    ...
    for (let i = 1; i < values.length; i++) {
      ...
      const personText = normalize(
        [row[COL.NOMBRE - 1], row[COL.PATERNO - 1], row[COL.MATERNO - 1]].join(' '),
      )

      // Coincide si TODAS las palabras buscadas están en el apellido de
      // familia (ej. "Sandoval Priego") o en el nombre completo de esa
      // persona (ej. "Renata Sandoval Priego"). Una sola palabra suelta (un
      // apellido común) ya no alcanza para traer resultados.
